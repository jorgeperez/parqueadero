...
function sec_session_start() {        //para iniciar sesion mas seguro 
...
function login($email, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT id_usuario, usuario, pass_usuario, salt_usuario
        FROM usuarios
       WHERE email_usuario = ?
        LIMIT 1")) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $mysqli) == true) {
                ...
                if ($db_password == $password) {
                    ...
                    $now = time();
                    $mysqli->query("INSERT INTO intentosesion (user_id, tiempo)
                                    VALUES ('$user_id', '$now')");
                    return false;
                }
            }
